...
// -----------------------------------------------
// Calculate stats split by turns (1st and 2nd)
// -----------------------------------------------
function getTurnStats(matches) {
  if (!matches) return [];
  ...
  Object.keys(matches).forEach(roundKey => {
    ...
    const isFirstTurnUserDef = round <= 19;
    ...
    roundMatches.forEach(match => {
      ...
      const homeResults = getResults(match.homeScore, match.awayScore);

      // Calculate results for away team
      const awayResults = getResults(match.awayScore, match.homeScore);

      if (isFirstTurnUserDef) {
        teamStats[match.homeTeam].points1stTurn += homeResults.points;
        teamStats[match.homeTeam].goals1stTurn += homeResults.goalsFor;

        teamStats[match.awayTeam].points1stTurn += awayResults.points;
        teamStats[match.awayTeam].goals1stTurn += awayResults.goalsFor;
      } else {
        teamStats[match.homeTeam].points2ndTurn += homeResults.points;
        teamStats[match.homeTeam].goals2ndTurn += homeResults.goalsFor;

        teamStats[match.awayTeam].points2ndTurn += awayResults.points;
        teamStats[match.awayTeam].goals2ndTurn += awayResults.goalsFor;
      }
    });
  });
  ...
  if (started && !(dateLimit && date > dateLimit)) {
    ...
    if (calculateHome) {
      ...
      if (!standings[homeTeam]) {
        // This it not expected to happen
        console.log('Home team not found: ' + homeTeam);

        // Raise error
        throw new Error(`Home team (${homeTeam}) not found: ${JSON.stringify(match)}`);
      }
    ...
    if (calculateAway) {
      ...
      if (!standings[awayTeam]) {
        // This it not expected to happen
        console.log('Away team not found: ' + awayTeam);

        // Raise error
        throw new Error(`Away team (${awayTeam}) not found: ${JSON.stringify(match)}`);
      }
